...
class Agent(nj.Module):
  ...
  def preprocess(self, obs):
    ...
    for key, value in obs.items():
      ...
      obs[key] = value
    # If is_terminal is not provided (e.g., during live inference), assume episode continues
    obs['cont'] = 1.0 - obs.get('is_terminal', jnp.zeros_like(obs['reward'])).astype(jnp.float32)
    return obs
...
class WorldModel(nj.Module):
  ...
  def imagine(self, policy, start, horizon):
    # If is_terminal not provided (e.g., during inference), assume episode continues
    first_cont = (1.0 - start.get('is_terminal', False)).astype(jnp.float32)
  ...
  def _metrics(self, data, dists, post, prior, losses, model_loss):
    ...
    if 'reward' in dists and not self.config.jax.debug_nans:
      stats = jaxutils.balance_stats(dists['reward'], data['reward'], 0.0)
