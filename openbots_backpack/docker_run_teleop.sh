...
IMAGE_NAME="openbots:teleop"

echo "Launching Docker container from image: $IMAGE_NAME"

docker run -it \
    --hostname ob_container_teleop \
    --network host \
    --gpus all \
    ...
    -v $(pwd)/../dreamer_results_local_noobs:/home/ob/dreamer_results_local_noobs \
    -v /dev/input:/dev/input \
    -v /media/maurits-heemskerk/69987a47-b840-4db7-9f8b-7cc05f14d09e5:/media/external_drive \
    --device-cgroup-rule='c 13:* rmw' \
    --group-add=995 \
    -w /home/ob/openbots_ws \
    --name openbots_container_teleop \
    $IMAGE_NAME \
    bash -c "sudo chmod -R 777 /media/external_drive 2>/dev/null; mkdir -p /media/external_drive/recorded_data_NoObs; exec bash"
