...
def populate_replay(replay, data_dir, shuffle_episodes=True, seed=None):
    ...
    for worker, ep_file in enumerate(episode_files):
        with h5py.File(ep_file, 'r') as f:
            ...
            # Use the exact goal that was used when computing stored rewards
            # (HER-relabelled). Must match spot.py._load_episode() exactly.
            if 'goal_position_x' in f.attrs and 'goal_position_y' in f.attrs:
                goal_pos = np.array(
                    [f.attrs['goal_position_x'], f.attrs['goal_position_y']],
                    dtype=np.float32)
            else:
                goal_pos = states[-1, :2].astype(np.float32)
