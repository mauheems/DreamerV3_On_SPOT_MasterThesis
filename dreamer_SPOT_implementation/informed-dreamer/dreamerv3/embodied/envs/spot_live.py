...
class SPOTLive(embodied.Env):
    ...
    def _build_obs(self):
        ...
        # Geofence: Clamp goal to 5m radius from episode start, with drift tolerance
        if 'goal' in self._obs_keys and 'position' in self._obs_keys:
            max_dist = 5.0  # 5m safe zone from starting position
            drift_margin = 0.5  # 0.5m margin for sensor drift
            
            # Get current position relative to episode start (already compensated for rotation)
            body_dist = np.linalg.norm(obs['position'])
            
            # Clamp goal to safe radius
            goal_dist = np.linalg.norm(obs['goal'])
            if goal_dist > max_dist:
                obs['goal'] = obs['goal'] * (max_dist / max(goal_dist, 0.01))
            
            # Penalty if robot goes outside safe zone
            if body_dist > max_dist + drift_margin:
                penalty = (body_dist - (max_dist + drift_margin)) * 0.5  # Scaled penalty
                obs['reward'] = obs.get('reward', 0.0) - penalty
