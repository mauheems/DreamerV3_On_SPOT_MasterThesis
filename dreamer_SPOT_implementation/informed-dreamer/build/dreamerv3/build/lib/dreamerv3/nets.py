...
class ImageEncoderResnet(nj.Module):
  ...
  def __call__(self, x):
    stages = int(np.log2(max(x.shape[-3:-1])) - np.log2(self._minres))
    depth = self._depth
...
class ImageDecoderResnet(nj.Module):
  ...
  def __call__(self, x):
    stages = int(np.log2(max(self._shape[-3:-1])) - np.log2(self._minres))
    depth = self._depth * 2 ** (stages - 1)
    x = jaxutils.cast_to_compute(x)
    h = int(np.ceil(self._shape[-3] / (2 ** stages)))
    w = int(np.ceil(self._shape[-2] / (2 ** stages)))
    x = self.get('in', Linear, (h, w, depth))(x)
    for i in range(stages):
      for j in range(self._blocks):
        skip = x
        kw = {**self._kw, 'preact': True}
      ...
      if self._resize == 'stride':
        ...
        raise NotImplementedError(self._resize)
    if x.shape[1] > self._shape[0] or x.shape[2] > self._shape[1]:
      padh = max(0, (x.shape[1] - self._shape[0]) / 2)
      padw = max(0, (x.shape[2] - self._shape[1]) / 2)
      endh = x.shape[1] - int(np.floor(padh))
      endw = x.shape[2] - int(np.floor(padw))
      x = x[:, int(np.ceil(padh)):endh, int(np.ceil(padw)):endw, :]
    # print(x.shape)
