...
def main(argv=None):
  ...
  args = embodied.Config(
      **config.run, logdir=config.logdir,
      batch_steps=config.batch_size * config.batch_length, env=config.env)
  print(config)
  ...
  try:
    ...
    if args.script == 'train':
      ...
      replay = make_replay(config, logdir / 'replay')
      env = make_envs(config)  # used only for obs_space / act_space
      cleanup.append(env)
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.train_offline(agent, env, replay, logger, args)
...
def make_env(config, **overrides):
  ...
  ctor = {
      ...
      'spot': 'embodied.envs.spot:SPOT',
      'spotlive': 'embodied.envs.spot_live:SPOTLive',
  }[suite]
  ...
  # Allow passing extra env.<suite> arguments from config, e.g. env.spot.data_dir
  if hasattr(config, f'env.{suite}'):
    kwargs.update(getattr(config, f'env.{suite}'))
