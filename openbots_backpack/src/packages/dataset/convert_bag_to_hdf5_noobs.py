...
class BagConverter:
    ...
    def _write_hdf5(self):
        ...
        # === Generate Goal Position and Early-Goal Timestamp ===
        # 25% probability: set goal to random position in 5m radius from start
        # 10% probability: set goal to T-5 instead of T (early goal for terminal condition learning)
        states_arr = np.array(self.episode['states'])
        T = len(states_arr)
        
        rng = np.random.RandomState(hash(self.bag_path) % (2**31))  # Deterministic seeding
        
        goal_position = states_arr[-1, :2].copy()  # Default: end position
        goal_timestep = T - 1  # Default: last timestep
        goal_type = 'end_position'
        
        if rng.rand() < 0.25:  # 25% chance: random goal
            # Random goal in 5m radius from start position
            angle = rng.uniform(0, 2 * np.pi)
            radius = rng.uniform(0, 5.0)
            goal_position = np.array([
                radius * np.cos(angle),
                radius * np.sin(angle)
            ], dtype=np.float32)
            goal_type = 'random_5m_radius'
            print(f"  Random goal set: ({goal_position[0]:.2f}, {goal_position[1]:.2f}) at distance {radius:.2f}m")
        elif rng.rand() < 0.10:  # 10% of remaining: early goal at T-8
            goal_timestep = max(0, T - 9)  # T-8 (subtract 1 for indexing)
            goal_position = states_arr[goal_timestep, :2].copy()
            goal_type = 'early_goal_t_minus_8'
            print(f"  Early goal set: timestep {goal_timestep}/{T-1}, position ({goal_position[0]:.2f}, {goal_position[1]:.2f})")
        else:
            print(f"  Default goal: end position ({goal_position[0]:.2f}, {goal_position[1]:.2f})")
        ...
        for attempt in range(max_retries):
            try:
                with h5py.File(out_name, 'w') as f:
                    # Save goal metadata
                    f.attrs['goal_type'] = goal_type
                    f.attrs['goal_position_x'] = float(goal_position[0])
                    f.attrs['goal_position_y'] = float(goal_position[1])
                    f.attrs['goal_timestep'] = int(goal_timestep)
                    f.attrs['episode_length'] = T
